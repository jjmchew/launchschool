...
function toString(date) {
  ...
  // return `${date.getYear()}-${date.getMonth()}-${date.getDay()}`; // this should probably be date.getDate(), getYear is deprecated
  return `${date.getFullYear()}-${pad(String(date.getMonth() + 1))}-${pad(String(date.getDate()))}`;
}
...
function getCompatibleEvents(classes, calendar) {
  function isAvailable(date) {
    ...
    console.log('toString(date): ', dateStr, date);
