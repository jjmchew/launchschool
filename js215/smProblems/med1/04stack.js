...
input
- string : contains 'minilang' commands

output
- dependant on the minilang commands provided

rules
- register is a variable
  - initial value `0`
- stack is a list of values; like an array
  - initial value `[]`
- n (an integer) : put that integer value in `register`
- PUSH: push register value onto stack, leave value in register
- ADD: pop value from stack add it to register, store the result in register
...

- assume no invalid commands / programs are given
- n will only be integers

approach
- initialize stack and register based on initial values
- split input string by ' ' into input commands to parse
- iterate through commands 1 by 1
  - utilize switch/case statement to parse command and execute as per description (helper fct)
*/
...
function minilang(inputStr) {
  ...
  let register = 0;
  let stack = [];
  inputStr.split(' ').forEach(command => {
    parseCommand(command);
    // console.log(command, 'stack: ', stack, '  register', register);
  });
  console.log('==============');
}