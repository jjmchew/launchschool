...
/*
elapsed:  1 hr : and basically gave up since I couldn't keep track of the indexes
...
- 'searchTerm' : an item to search 'collection' for

output
- an integer index of an element in 'collection' OR -1 (if 'searchTerm' can't be found in 'collection')

rules
- 'collection' will always be sorted
- assume that all elements in 'collection' are the same type (otherwise can't sort)?
- assume no sparse arrays?  elements might be other types?

approach
- binary search:
  - create a helper fct 'search' (takes a start index, end index) to search 'collection' with
  - get 'middleIndex' (round down if necessary)
  - if middle value is 'searchTerm', return 'middleIndex'
  - if 'searchTerm' > middle value, call search from 'middleIndex + 1' to collection.length - 1
  - if 'searchTerm' < middle value, call search from 0 to 'middleIndex - 1'

start  end     middleIndex      middleValue
[1, 2, 3, 4], 0
0      3       1                2
0      1       0                1
0      0       return -1
...
function binarySearch(collection, searchTerm) {
  ...
  while (low <= high) {
    ...
    else if (collection[mid] < searchTerm) low = mid + 1;
    else high = mid - 1;
  }
