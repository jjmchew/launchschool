...
class LinkedListTest < Minitest::Test
  ...
  def test_each
    ...
    new_arr = []
    new_list = @list.each { |element| new_arr << element }
    assert_same(@list, new_list)
  end

  def test_map
    new_list = @list.map { |element| element.to_s }
    assert_equal(["3", "2", "1"], new_list.to_a)
  end

  def test_map_2
    new_list = @list.map { |element| element + 10 }
    assert_equal([13, 12, 11], new_list.to_a)
  end

  def test_select
    new_list = @list.select { |element| element.odd? }
    assert_equal([3, 1], new_list.to_a)
  end
