...
input
  - large of block of text - assume as a string
output
  - print the longest sentence
  - # of words in the longest sentence
assumptions / constraints
  - sentences always end in periods (.), exclamation points (!), or question marks (?)

data structure
  - text: string
  - sentences:  array of strings
  - words (in sentences): array of strings
  - word count (for each sentence):  hash?  

algorithm
  - split text into sentences  (get_sentences)
      - iterate across each character in the text
      - if that character is NOT [. ! ?] then add it to a 'sentence'
      - if that character IS [. ! ?] then add it, 'complete' the sentence, start a new 'sentence'
          - collect each sentence in an array of strings
      - continue scanning until end of string
      ( consider leading space on start of next sentence? )
  - count the words in each sentence
  - store the count of words in that sentence
  - search for sentence with the highest count
...
def count_sentences(text)
  ...
  current = { longest: 0, index: nil }
  sentences.each_with_index do |sentence, index|
    if current[:longest] < sentence.split.length
      current[:longest] = sentence.split.length
      current[:index] = index
    end
  end
  puts "The longest sentence is:"
  puts sentences[current[:index]]
  puts "This sentence has #{current[:longest]} words."
end

count_sentences(text2)