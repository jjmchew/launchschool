...
=begin
The calculations did not return the desired result since integers were being used.  In particular, the method `idf` requires at least 1 of the local variables `number_of_documents` or `number_of_documents_with_term` to be converted to a float number so that the expression can be properly evaluated prior to being passed as an argument to `Math.log`.  Adding the `.to_f` method to the evaluated value of `number_of_documents` fixes the problem.

The additional problem is the potential to divide by `0` if the `number_of_documents_with_term` is 0.
=end