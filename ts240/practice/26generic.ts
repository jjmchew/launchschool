...
{
  function filterByType(arr: Array<number | string | boolean>, type: string): Array<number | string | boolean> {
    return arr.filter(el => {
      const elType = typeof el;
      if (elType === 'string' && elType === type) return true;
      else if (elType === 'number' && elType === type) return true;
      else if (elType === 'boolean' && elType === type) return true;
    });
  }
